# Utils - 工具函数模块

**版本**: 1.0.0-alpha  
**状态**: ✅ 核心功能已完成  
**最后更新**: 2025-01-23

---

## 📋 概述

Utils 模块提供 SDK 内部使用的工具函数，包括地址转换、编码解码等辅助功能。所有工具函数都是独立的，不依赖任何 WES 内部包。

---

## 🏗️ 模块架构

```
┌─────────────────────────────────────────┐
│            Utils 模块架构                │
└─────────────────────────────────────────┘

utils/
  │
  └─> address.go
      │
      ├─> AddressBytesToBase58()      ✅
      ├─> AddressBase58ToBytes()     ✅
      ├─> AddressHexToBase58()        ✅
      └─> AddressBase58ToHex()        ✅
```

---

## 🔧 核心功能

### 地址转换

#### Base58Check 编码格式

```
┌─────────────────────────────────────────┐
│      WES 地址格式 (Base58Check)        │
└─────────────────────────────────────────┘

完整地址结构:
  [版本字节(1)] + [地址哈希(20)] + [校验和(4)] = 25 字节
       ↓              ↓              ↓
     Base58Check 编码
       ↓
  WES 地址字符串 (Base58)
```

**版本字节**: `0x1C` (WESP2PKHVersion)

**校验和**: 双重 SHA256 的前 4 字节

#### 地址转换流程

```
┌─────────────────────────────────────────┐
│        地址转换流程                     │
└─────────────────────────────────────────┘

20 字节地址哈希
    ↓
添加版本字节 (0x1C)
    ↓
计算校验和 (SHA256(SHA256(versioned)))
    ↓
Base58 编码
    ↓
WES 地址 (Base58)
```

---

## 📖 API 参考

### AddressBytesToBase58

将 20 字节地址转换为 Base58Check 编码。

```go
func AddressBytesToBase58(addressBytes []byte) (string, error)
```

**参数**:
- `addressBytes`: 20 字节地址哈希

**返回**:
- `string`: Base58Check 编码的地址
- `error`: 错误信息

**示例**:
```go
addressBytes := []byte{0x12, 0x34, ...} // 20 字节
base58Addr, err := utils.AddressBytesToBase58(addressBytes)
if err != nil {
    log.Fatal(err)
}
fmt.Printf("地址: %s\n", base58Addr) // 输出: CUQ3g6P5WmFN289pPn7AAhnQ3T2cZRv2BR
```

### AddressBase58ToBytes

将 Base58Check 编码地址转换为 20 字节地址哈希。

```go
func AddressBase58ToBytes(base58Addr string) ([]byte, error)
```

**参数**:
- `base58Addr`: Base58Check 编码的地址

**返回**:
- `[]byte`: 20 字节地址哈希
- `error`: 错误信息

**示例**:
```go
base58Addr := "CUQ3g6P5WmFN289pPn7AAhnQ3T2cZRv2BR"
addressBytes, err := utils.AddressBase58ToBytes(base58Addr)
if err != nil {
    log.Fatal(err)
}
fmt.Printf("地址字节: %x\n", addressBytes) // 输出: 1234...
```

### AddressHexToBase58

将十六进制地址字符串转换为 Base58Check 编码。

```go
func AddressHexToBase58(hexAddr string) (string, error)
```

**参数**:
- `hexAddr`: 十六进制地址字符串（可带或不带 `0x` 前缀）

**返回**:
- `string`: Base58Check 编码的地址
- `error`: 错误信息

**示例**:
```go
hexAddr := "0x1234567890abcdef1234567890abcdef12345678"
base58Addr, err := utils.AddressHexToBase58(hexAddr)
```

### AddressBase58ToHex

将 Base58Check 编码地址转换为十六进制字符串。

```go
func AddressBase58ToHex(base58Addr string) (string, error)
```

**参数**:
- `base58Addr`: Base58Check 编码的地址

**返回**:
- `string`: 十六进制地址字符串（不带 `0x` 前缀）
- `error`: 错误信息

**示例**:
```go
base58Addr := "CUQ3g6P5WmFN289pPn7AAhnQ3T2cZRv2BR"
hexAddr, err := utils.AddressBase58ToHex(base58Addr)
// 输出: 1234567890abcdef1234567890abcdef12345678
```

---

## 🔒 实现细节

### Base58Check 编码算法

```
┌─────────────────────────────────────────┐
│      Base58Check 编码算法                │
└─────────────────────────────────────────┘

1. 构建载荷
   [版本字节(0x1C)] + [地址哈希(20字节)]

2. 计算校验和
   checksum = SHA256(SHA256(payload))[:4]

3. 构建完整数据
   [载荷] + [校验和(4字节)] = 25 字节

4. Base58 编码
   使用 btcutil/base58 库编码
```

### 校验和验证

```
┌─────────────────────────────────────────┐
│        校验和验证流程                    │
└─────────────────────────────────────────┘

Base58 解码
    ↓
分离载荷和校验和
    ↓
计算期望校验和
    SHA256(SHA256(payload))[:4]
    ↓
比较校验和
    ↓
验证通过/失败
```

---

## 🎯 使用场景

### 1. 查询 UTXO

```go
// 将字节地址转换为 Base58（wes_getUTXO 需要 Base58 地址）
fromAddrBytes := wallet.Address()
fromAddrBase58, err := utils.AddressBytesToBase58(fromAddrBytes)
if err != nil {
    return err
}

// 调用 API
result, err := client.Call(ctx, "wes_getUTXO", []interface{}{fromAddrBase58})
```

### 2. 地址格式转换

```go
// 从十六进制转换为 Base58
hexAddr := "0x1234567890abcdef1234567890abcdef12345678"
base58Addr, _ := utils.AddressHexToBase58(hexAddr)

// 从 Base58 转换为十六进制
hexAddr, _ := utils.AddressBase58ToHex(base58Addr)
```

---

## ⚠️ 注意事项

### 1. 地址长度

- ✅ 输入地址必须是 **20 字节**
- ✅ Base58Check 编码后长度为 **25 字节**（编码前）
- ✅ Base58 字符串长度约为 **34 字符**

### 2. 版本字节

- ✅ WES 使用版本字节 `0x1C` (WESP2PKHVersion)
- ✅ 与 Bitcoin 主网版本 (`0x00`) 不同
- ✅ 确保使用正确的版本字节

### 3. 错误处理

所有函数都会进行严格的参数验证：

```go
// 地址长度验证
if len(addressBytes) != 20 {
    return "", fmt.Errorf("invalid address length: expected 20 bytes, got %d", len(addressBytes))
}

// 校验和验证
if !equalBytes(checksum, expectedChecksum) {
    return nil, fmt.Errorf("invalid checksum")
}
```

---

## 📚 相关文档

- [主 README](../README.md) - SDK 总体文档
- [Client 文档](../client/README.md) - 客户端文档

---

**最后更新**: 2025-01-23  
**维护者**: WES Core Team

