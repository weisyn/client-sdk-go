# Wallet - 钱包功能模块

**版本**: 1.0.0-alpha  
**状态**: ✅ 核心功能已完成  
**最后更新**: 2025-01-23

---

## 📋 概述

Wallet 模块提供密钥管理、交易签名等核心功能。所有业务服务都通过 Wallet 接口进行签名，确保私钥不离开钱包，符合安全最佳实践。

---

## 🏗️ 架构设计

### 模块架构图

```
┌─────────────────────────────────────────────────────────┐
│                  Wallet 模块架构                         │
└─────────────────────────────────────────────────────────┘

┌──────────────────┐
│  Wallet 接口     │  (wallet.Wallet)
│                  │
│  • Address()     │
│  • SignTransaction() │
│  • SignMessage() │
│  • PrivateKey()  │
└──────────────────┘
        ↑
        │ 实现
┌──────────────────┐      ┌──────────────────┐
│  SimpleWallet    │      │  KeystoreManager │
│                  │      │                  │
│  • 内存存储      │      │  • 加密存储      │
│  • 开发/测试用   │      │  • PBKDF2        │
│                  │      │  • AES-256-GCM   │
└──────────────────┘      └──────────────────┘
        ↑                          ↑
        │                          │
┌──────────────────┐      ┌──────────────────┐
│  私钥管理        │      │  文件存储        │
│                  │      │                  │
│  • ECDSA         │      │  • JSON 格式     │
│  • secp256k1     │      │  • 加密私钥      │
└──────────────────┘      └──────────────────┘
```

### Wallet 在交易流程中的位置

```
┌─────────────────────────────────────────────────────────┐
│            Wallet 在交易流程中的作用                     │
└─────────────────────────────────────────────────────────┘

业务服务层
  │
  ├─> 构建未签名交易
  │   └─> unsignedTxBytes
  │
  ↓
Wallet 层 (wallet/)
  │
  ├─> wallet.SignTransaction(unsignedTxBytes)
  │   │
  │   ├─> 1. 解析交易
  │   ├─> 2. 计算交易哈希
  │   ├─> 3. ECDSA 签名
  │   └─> 4. 序列化签名
  │
  ↓
已签名交易
  │
  └─> signedTxBytes → "0x..." (hex)
      │
      ↓
客户端层
  │
  └─> client.SendRawTransaction(signedTxHex)
      │
      ↓
节点 API
```

### 密钥派生流程

```
┌─────────────────────────────────────────────────────────┐
│              地址派生流程（简化版）                      │
└─────────────────────────────────────────────────────────┘

私钥 (32 bytes)
  │
  ├─> [当前实现] 使用私钥 D 值的前 20 字节
  │
  └─> [标准实现] (未来)
      │
      ├─> 1. 从私钥导出公钥 (secp256k1)
      ├─> 2. SHA256(公钥)
      ├─> 3. RIPEMD160(SHA256(公钥))
      ├─> 4. 添加版本字节
      ├─> 5. 计算校验和
      └─> 6. Base58Check 编码
          │
          └─> WES 地址 (Base58)
```

---

## 🔑 核心接口

### Wallet 接口

```go
type Wallet interface {
    // Address 返回钱包地址（20 字节）
    Address() []byte
    
    // SignTransaction 签名交易
    SignTransaction(tx []byte) ([]byte, error)
    
    // SignMessage 签名消息
    SignMessage(msg []byte) ([]byte, error)
    
    // SignHash 签名哈希值
    SignHash(hash []byte) ([]byte, error)
    
    // PrivateKey 返回私钥（谨慎使用）
    PrivateKey() *ecdsa.PrivateKey
}
```

### 实现类型

#### SimpleWallet

**用途**: 开发、测试、简单应用

**特点**:
- ✅ 内存存储私钥
- ✅ 快速创建和使用
- ✅ 支持从私钥创建
- ⚠️ **不适合生产环境**

**使用场景**:
```
开发环境
  ├─> 快速测试
  ├─> 示例代码
  └─> 原型开发
```

#### KeystoreManager

**用途**: 生产环境、持久化存储

**特点**:
- ✅ 加密存储私钥
- ✅ PBKDF2 密钥派生
- ✅ AES-256-GCM 加密
- ✅ JSON 格式存储

**使用场景**:
```
生产环境
  ├─> 应用钱包
  ├─> 服务端应用
  └─> 需要持久化的场景
```

---

## 🔧 使用指南

### 1. 创建钱包

#### 方式1: 创建新钱包

```go
wallet, err := wallet.NewWallet()
if err != nil {
    log.Fatal(err)
}

address := wallet.Address()
fmt.Printf("新钱包地址: %x\n", address)
```

#### 方式2: 从私钥创建

```go
// 从十六进制私钥创建
privateKeyHex := "0x1234567890abcdef..."
wallet, err := wallet.NewWalletFromPrivateKey(privateKeyHex)
if err != nil {
    log.Fatal(err)
}

address := wallet.Address()
fmt.Printf("钱包地址: %x\n", address)
```

#### 方式3: 从 Keystore 加载

```go
keystore := wallet.NewKeystoreManager("/path/to/keystore")

// 加载钱包
wallet, err := keystore.LoadWallet("wallet.json", "password")
if err != nil {
    log.Fatal(err)
}
```

### 2. 签名交易

```go
// 1. 获取未签名交易（从业务服务）
unsignedTxBytes := []byte{...} // 从 buildTransaction 获取

// 2. 使用 Wallet 签名
signedTxBytes, err := wallet.SignTransaction(unsignedTxBytes)
if err != nil {
    log.Fatal(err)
}

// 3. 转换为十六进制
signedTxHex := "0x" + hex.EncodeToString(signedTxBytes)

// 4. 提交交易
result, err := client.SendRawTransaction(ctx, signedTxHex)
```

### 3. 签名消息

```go
message := []byte("Hello, WES!")

signature, err := wallet.SignMessage(message)
if err != nil {
    log.Fatal(err)
}

fmt.Printf("签名: %x\n", signature)
```

### 4. 在业务服务中使用

```go
// 方式1: 方法参数传递（推荐）
tokenService := token.NewService(client)
result, err := tokenService.Transfer(ctx, &token.TransferRequest{
    From:   wallet.Address(),
    To:     toAddr,
    Amount: 1000,
}, wallet) // 传入 wallet

// 方式2: 构造函数设置默认 wallet
tokenService := token.NewServiceWithWallet(client, wallet)
result, err := tokenService.Transfer(ctx, &token.TransferRequest{
    From:   wallet.Address(),
    To:     toAddr,
    Amount: 1000,
}) // 使用默认 wallet
```

---

## 🔒 安全考虑

### 1. 私钥管理策略

```
┌─────────────────────────────────────────┐
│        私钥管理策略                      │
└─────────────────────────────────────────┘

开发环境:
  SimpleWallet
    ├─> 内存存储
    ├─> 快速创建
    └─> ⚠️ 不持久化

生产环境:
  KeystoreManager
    ├─> 加密存储
    ├─> 密码保护
    └─> ✅ 安全持久化

硬件钱包 (未来):
  HardwareWallet
    ├─> 私钥不离开设备
    ├─> 硬件签名
    └─> ✅ 最高安全性
```

### 2. 签名安全

```
┌─────────────────────────────────────────┐
│        签名安全流程                      │
└─────────────────────────────────────────┘

1. 交易构建 (SDK 层)
   └─> 不包含私钥信息

2. Wallet 签名 (客户端)
   └─> 私钥不离开钱包

3. 提交交易 (API)
   └─> 只传输已签名交易

4. 节点验证
   └─> 验证签名有效性
```

### 3. Keystore 加密

```
┌─────────────────────────────────────────┐
│        Keystore 加密流程                │
└─────────────────────────────────────────┘

私钥 (32 bytes)
    ↓
PBKDF2 (密码 + Salt)
    ↓
派生密钥 (32 bytes)
    ↓
AES-256-GCM 加密
    ↓
加密私钥 + IV + Tag
    ↓
JSON 格式存储
    {
      "crypto": {
        "cipher": "aes-256-gcm",
        "kdf": "pbkdf2",
        ...
      }
    }
```

---

## 📊 实现状态

### SimpleWallet ✅

- ✅ 钱包创建
- ✅ 从私钥创建
- ✅ 交易签名
- ✅ 消息签名
- ✅ 地址派生（简化版）
- ⚠️ 地址派生需要完善（当前使用私钥 D 值）

### KeystoreManager ✅

- ✅ Keystore 创建
- ✅ 钱包保存
- ✅ 钱包加载
- ✅ PBKDF2 密钥派生
- ✅ AES-256-GCM 加密
- ✅ JSON 格式存储

---

## 🚀 未来改进

### 1. 完善地址派生

```
当前: 使用私钥 D 值的前 20 字节（简化）

未来: 标准地址派生
  ├─> secp256k1 公钥导出
  ├─> SHA256 + RIPEMD160
  ├─> Base58Check 编码
  └─> WES 标准地址格式
```

### 2. 支持更多钱包类型

- [ ] HD 钱包（BIP39 助记词）
- [ ] 硬件钱包接口
- [ ] 多签名钱包

### 3. 增强安全性

- [ ] 硬件安全模块 (HSM) 支持
- [ ] 密钥分片存储
- [ ] 签名审计日志

---

## 📚 API 参考

### Wallet 接口

```go
type Wallet interface {
    // Address 返回钱包地址（20 字节）
    Address() []byte
    
    // SignTransaction 签名交易
    // 参数: 未签名交易（字节数组）
    // 返回: 已签名交易（字节数组）
    SignTransaction(tx []byte) ([]byte, error)
    
    // SignMessage 签名消息
    // 参数: 消息（字节数组）
    // 返回: 签名（字节数组，r||s 格式，64 字节）
    SignMessage(msg []byte) ([]byte, error)
    
    // SignHash 签名哈希值
    // 参数: 哈希值（32 字节）
    // 返回: 签名（字节数组，r||s 格式，64 字节）
    SignHash(hash []byte) ([]byte, error)
    
    // PrivateKey 返回私钥（谨慎使用）
    // ⚠️ 仅用于高级场景，生产环境应避免使用
    PrivateKey() *ecdsa.PrivateKey
}
```

### 构造函数

```go
// NewWallet 创建新钱包
func NewWallet() (Wallet, error)

// NewWalletFromPrivateKey 从私钥创建钱包
func NewWalletFromPrivateKey(privateKeyHex string) (Wallet, error)

// NewKeystoreManager 创建 Keystore 管理器
func NewKeystoreManager(keystoreDir string) *KeystoreManager
```

---

## 🔗 相关文档

- [主 README](../README.md) - SDK 总体文档
- [Services 文档](../services/README.md) - 业务服务文档
- [架构设计文档](../../SDK_DESIGN.md) - SDK 设计文档

---

**最后更新**: 2025-01-23  
**维护者**: WES Core Team
